...
async function testCatalogIntent() {
    ...
    // Test 4: 6 Products (Edge Case - currently fails)
    console.log('Test 4: 6 Products (Should list them, currently asks generic question)');
    const sixProducts = [
        ...mockProducts,
    ...
    ] as any; // Cast to avoid defining all fields

    const context4 = {
        ...baseContext,
        intent: { intent: 'disponibilidad', confidence: 0.8, entities: {} }, // User asks "que tienen"
        availableProducts: sixProducts
    };

    const response4 = await responseService.generateResponse(context4);
    console.log('Response:', response4.response);

    if (response4.response.includes('Product 6')) {
        console.log('✅ Passed: Listed 6 products\n');
    } else {
        console.log('❌ Failed: Did not list products (Likely asked generic question)\n');
    }

    // Test 5: Colloquial "Let me see" (Intent Check)
    console.log('Test 5: "dejame ver" (Should be detected as ver_catalogo)');
    // Note: We can't easily test intent detection here without mocking the detector properly or integrating it. 
    // But we can verify if we passed 'ver_catalogo' manually, does it handle it?
    // We will assume the intent detector *will* be updated. Here we test if *given* ver_catalogo, it lists 6 products.
    ...
    const response5 = await responseService.generateResponse(context5);
    console.log('Response:', response5.response);

    if (response5.response.includes('Product 6')) {
        console.log('✅ Passed: Listed 6 products for catalog intent\n');
    } else {
        console.log('❌ Failed: Did not list products (Likely showed categories or generic)\n');
    }
}
